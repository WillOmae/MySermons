...
namespace AppUI
{
    ...
    public class RichTextBoxEx : RichTextBox
    {
        ...
        private void FxnGetVerse(string szVerse, int position)
        {
            ...
            {
                for (int i = 0; i < readXMLBible.listOfBibleTextInfo.Count; i++)
                {
                    XMLBible.BIBLETEXTINFO BTI = readXMLBible.listOfBibleTextInfo[i];
                    MyDelegate myDelegate = new MyDelegate(FxnInsertLinks);
                    try
                    {
                        if (i != (readXMLBible.listOfBibleTextInfo.Count - 1))
                        {
